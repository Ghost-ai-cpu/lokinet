...
{
  const char *
  worktipsnetEnv(string_view suffix)
  {
    std::string env;
    env.reserve(8 + suffix.size());
    env.append("WORKTIPSNET_"s);
  ...
  {
    if(const char *ptr = worktipsnetEnv(envNameSuffix))
      return ptr;
  ...
  WorktipsdConfig::fromSection(string_view key, string_view val)
  {
    ...
    {
      worktipsdRPCAddr = str(val);
    }
    if(key == "username")
    {
      worktipsdRPCUser = str(val);
    }
    if(key == "password")
    {
      worktipsdRPCPassword = str(val);
    }
  }
